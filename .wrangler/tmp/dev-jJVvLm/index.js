...
// .wrangler/tmp/bundle-N7CS12/checked-fetch.js
...
var JWKS = null;
var jwksCreatedAt = 0;
var JWKS_MAX_AGE_MS = 5 * 60 * 1e3;
async function verifySupabaseToken(token, supabaseUrl) {
...
app.post("/api/teams/:id/members", async (c) => {
  ...
  const { user_id: memberUserId } = await c.req.json();
  const memberId = crypto.randomUUID();
  try {
...
app.post("/api/teams/join", async (c) => {
  ...
  try {
    const ownerProfile = await c.env.DB.prepare(`
        ...
        `).bind(owner_custom_id.toUpperCase()).first();
    if (!ownerProfile) {
...
async function getUserTeamId(db, userId, requestedTeamId) {
  if (requestedTeamId) {
    const membership2 = await db.prepare(`
            SELECT tm.team_id
            FROM team_members tm
            WHERE tm.user_id = ? AND tm.team_id = ?
        `).bind(userId, requestedTeamId).first();
    if (membership2) {
      return membership2.team_id;
    }
...
__name(getUserTeamId, "getUserTeamId");
...
app.put("/api/products/:id", async (c) => {
  ...
  try {
    ...
    const product = await c.env.DB.prepare("SELECT team_id FROM products WHERE id = ?").bind(id).first();
...
app.get("/api/stock/movements", async (c) => {
  ...
  try {
    ...
    const results = await c.env.DB.prepare(`
            ...
            LIMIT 100
        `).bind(teamId).all();
    const movements = results.results.map((m) => ({
      ...m,
...
// .wrangler/tmp/bundle-N7CS12/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-N7CS12/middleware-loader.entry.ts
